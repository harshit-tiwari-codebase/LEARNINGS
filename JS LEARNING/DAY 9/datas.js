...
let filtered = products.filter(products=>products.price>50).sort((a,b)=>(b.price-a.price)).map((products) =>({price:products.price,name:products.name}));
console.log(filtered);


let total = products.reduce((accumulator,currenValue)=>{
    return accumulator+ currenValue.price;
},0);

console.log(total);
let s1 = new Set(products);
console.log(s1);
for (let num of s1){
    console.log(num)
}